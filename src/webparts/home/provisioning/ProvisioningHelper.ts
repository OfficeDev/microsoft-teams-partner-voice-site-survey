        ...
        if (listItemCount == 0) {
          ...
          switch (listname) {
            case Constants.MasterList:
              ...
              break;
            case Constants.RefreshDataList:
              //Create master data
              for (let j = 0; j < masterDataToAdd.length; j++) {
                listContext.items.inBatch(batchProcess).add(
                  {
              ...
              break;
            case Constants.AssessmentsMaster:
              //Create master data
              for (let j = 0; j < masterDataToAdd.length; j++) {
                listContext.items.inBatch(batchProcess).add(
                  {
                    Title: masterDataToAdd[j]["Title"],
                    AssessmentColumn: masterDataToAdd[j]["AssessmentColumn"],
                    InfoIconText: masterDataToAdd[j]["InfoIconText"],
                  },
                  entityTypeFullName
                );
              }
